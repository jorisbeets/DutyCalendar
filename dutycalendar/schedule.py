...
class Schedule:
    ...
    days_on: list = field(default_factory=list)
    days_off: list = field(default_factory=list) 
    startdates: list = field(default_factory=list)

    def start_dates(self, count:int) -> list:
        # Create rruleset() that has two rule if there are two items in the days_off list. Can still be modified to accomodate flexing to len of list.
        # for i in range(len(days_off):  etc etc etc)
        ...
        self.startdates.rrule(rrule(
            ...
            interval=(self.days_off[0]+self.days_on[0])
            ))
        if len(self.days_off) > 1:
             # RRule No2: deze wordt gedaan als de lijsten langer zijn dan 1.
            self.startdates.rrule(rrule(
                freq=DAILY,
                dtstart=self.startdate[1],
                count =count,#deze kan ook met until gedaan.
                interval=(self.days_off[1]+self.days_on[1])
                ))
